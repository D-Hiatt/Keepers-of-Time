...
namespace Server.Items
{
    public class QuestNote : XmlQuestToken
    {
        ...
        public QuestNote()
            : base(0x14EE)
        {
            this.Name = "A quest note";
            this.TitleString = "A quest note";
        }

        public QuestNote(Serial serial)
            : base(serial)
        {
        }
    ...
    public class OriginalQuestNote : XmlQuestToken
    {
        ...
        public OriginalQuestNote(Serial serial)
            : base(serial)
        {
        }
    ...
    public class QuestNoteGump : Gump
    {
            ...
            : base(0, 0)
        {
            ...
            // title string
            this.AddHtml(55, 10, 200, 37, QuestNoteGump.HtmlFormat(note.TitleString, note.TitleColor), false, false);
            // text string
            this.AddHtml(40, 41, 225, 70 * note.Size, QuestNoteGump.HtmlFormat(note.NoteString, note.TextColor), false, false);
            
            // add the quest status gump button
            this.AddButton(40, 50 + note.Size * 70, 0x037, 0x037, 1, GumpButtonType.Reply, 0);
        }
        ...
        public override void OnResponse(Server.Network.NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                XmlQuestStatusGump g = new XmlQuestStatusGump(this.m_Note, this.m_Note.TitleString);
                from.SendGump(g);
            }
        }
    }
