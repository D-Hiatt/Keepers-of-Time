...
{
    public class TeamKotHGump : Gump
    {
        ...
        private readonly TeamKotHGauntlet m_gauntlet;
        private readonly ArrayList m_WorkingList;
        private readonly int viewpage;
        public TeamKotHGump(TeamKotHGauntlet gauntlet, Mobile from)
            : this(gauntlet, from, 0)
        {
        }

        public TeamKotHGump(TeamKotHGauntlet gauntlet, Mobile from, int page)
            : base(20, 30)
        {
            if (from == null || gauntlet == null || gauntlet.Deleted || gauntlet.Challenger == null)
                return;

            from.CloseGump(typeof(TeamKotHGump));
            ...
            this.AddLabel(100, 10, 0, XmlPoints.GetText(from, 200600));  // "Team KotH Challenge"
            ...
            if (gauntlet.Participants != null)
            {
                ...
                {
                    TeamKotHGauntlet.ChallengeEntry entry = (TeamKotHGauntlet.ChallengeEntry)this.m_WorkingList[i];
        ...
        public static void RefreshAllGumps(TeamKotHGauntlet gauntlet, bool force)
        {
            if (gauntlet.Participants != null)
            {
                foreach (TeamKotHGauntlet.ChallengeEntry entry in gauntlet.Participants)
                {
                    if (entry.Participant != null)
                    {
                        if (force || entry.Participant.HasGump(typeof(TeamKotHGump)))
                        {
                            entry.Participant.SendGump(new TeamKotHGump(gauntlet, entry.Participant));
                        }
                    }
                }
            }
            ...
            {
                if (force || gauntlet.Challenger.HasGump(typeof(TeamKotHGump)))
                {
                    gauntlet.Challenger.SendGump(new TeamKotHGump(gauntlet, gauntlet.Challenger));
                }
            }
        }
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch ( info.ButtonID )
            {
                case 1:
                    ...
                    state.Mobile.SendGump(new TeamKotHGump(this.m_gauntlet, state.Mobile, this.viewpage));
                    break;
                case 10:
                    // Entry fee
                    int val = 0;
                ...
                case 12:
                    ...
                    state.Mobile.SendGump(new TeamKotHGump(this.m_gauntlet, state.Mobile, page));
                    break;
                case 13:
                    // page down
                    // try doing the default for non-participants
                ...
                case 300:
                    ...
                    bool complete = true;
                    foreach (TeamKotHGauntlet.ChallengeEntry entry in this.m_WorkingList)
                    {
                        if (entry != null)
                        {
                            Mobile m = entry.Participant;
                ...
                default:
                    {
                        ...
                        if (info.ButtonID >= 4000)
                        {
                            ...
                            {
                                TeamKotHGauntlet.ChallengeEntry entry = (TeamKotHGauntlet.ChallengeEntry)this.m_WorkingList[selection];

                                // find the master participants list entry with the same participant
                                if (this.m_gauntlet.Participants != null)
                                {
                                    TeamKotHGauntlet.ChallengeEntry forfeitentry = null;

                                    foreach (TeamKotHGauntlet.ChallengeEntry masterentry in this.m_gauntlet.Participants)
                                    {
                        ...
                        if (info.ButtonID >= 2000)
                        {
                            ...
                            if (selection < this.m_WorkingList.Count)
                            {
                                ...
                                if (this.m_gauntlet.Participants != null)
                                {
                                    TeamKotHGauntlet.ChallengeEntry kickentry = null;
                                    ...
                                    if (kickentry != null)
                                    {
                                        ...
                                        if (kickentry.Participant != null)
                                        {
                                            ...
                                            kickentry.Participant.SendGump(new TeamKotHGump(this.m_gauntlet, kickentry.Participant, this.viewpage));
                                        }
                                    }
                                }
                            ...
                            // refresh all gumps
                            RefreshAllGumps(this.m_gauntlet, true);
                            //state.Mobile.SendGump( new TeamKotHGump( m_gauntlet, state.Mobile));
                        }
        ...
        {
            public TeamsGump(TeamKotHGauntlet gauntlet, Mobile from)
                : base(350,30)
            { 
                ...
                this.AddLabel(60, 10, 0, XmlPoints.GetText(from, 200601));   // "KotH Team Status"
        ...
        private class MemberTarget : Target
        {
            ...
            private readonly TeamKotHGauntlet m_gauntlet;
            public MemberTarget(TeamKotHGauntlet gauntlet, ArrayList list)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is Mobile && ((Mobile)targeted).Player)
                {
                    ...
                    if (this.m_gauntlet.Participants != null)
                    {
                        foreach (TeamKotHGauntlet.ChallengeEntry entry in this.m_gauntlet.Participants)
                        {
                    ...
                    this.m_list.Add(new TeamKotHGauntlet.ChallengeEntry(pm));
                    ...
                    //from.SendGump( new TeamKotHGump( m_gauntlet, from));
                }
            }
        }
    }
