...
namespace Server.Engines.XmlSpawner2
{
    ...
    public abstract class BaseChallengeEntry : IChallengeEntry
    {
        ...
        public virtual Mobile Participant
        {
            get
            {
        ...
        public virtual ChallengeStatus Caution
        {
            get
            {
                return this.m_Caution;
            }
            set
            {
                this.m_Caution = value;
            }
        }
        ...
        public virtual DateTime LastCaution
        {
            get
            {
                return this.m_LastCaution;
            }
            set
            {
                this.m_LastCaution = value;
            }
        }
        public virtual int PageBeingViewed
        {
            get
            {
                return this.m_PageBeingViewed;
            }
            set
            {
                this.m_PageBeingViewed = value;
            }
        }
    ...
    public abstract class BaseChallengeGame : Item, IChallengeGame
    {
        ...
        public BaseChallengeGame(int itemid)
            : base(itemid)
        {
        }

        public BaseChallengeGame()
            : base(0)
        {
        }
        ...
        public ChallengeTimer GameTimer
        {
            get
            {
                return this.m_Timer;
        ...
        public bool IsInChallengeGameRegion
        {
            get
            {
                return this.m_IsInChallengeGameRegion;
            }
            set
            {
                this.m_IsInChallengeGameRegion = value;
            }
        }
        ...
        public virtual ArrayList Organizers
        {
            get
            {
                return null;
        ...
        public void ClearArena()
        {
            ...
            foreach (Mobile p in this.GetMobilesInRange(this.ArenaSize))
            {
                if (p == null)
                    continue;
            ...
            foreach (Mobile p in mlist)
            {
                for (int i = 0; i < 10; i++)
                {
                    int x = this.Location.X + (this.ArenaSize + i) * (Utility.RandomBool() ? 1 : -1);
                    int y = this.Location.Y + (this.ArenaSize + i) * (Utility.RandomBool() ? 1 : -1);
        ...
        public virtual bool CheckQualify(Mobile m)
        {
            ...
            if (a == null)
            {
                XmlPoints.SendText(this.Challenger, 100536, m.Name);   // "{0} does not qualify. No points support."
                return false;
            }
            ...
            if (a.HasChallenge)
            {
                XmlPoints.SendText(this.Challenger, 100537, m.Name);  // "{0} is already in a Challenge."
                return false;
            }
            ...
            {
                XmlPoints.SendText(this.Challenger, 100538, m.Name);  // "{0} cannot afford the Entry fee."
                return false;
            }
        ...
        public virtual void Forfeit(Mobile m)
        {
            ...
            XmlPoints.SendText(m, 100543, this.ChallengeName);   // "You dropped out of {0}"
            this.GameBroadcast(100544, m.Name);  // "{0} has dropped out."

            this.RefreshSymmetricNoto(m);
        ...
        public virtual void AwardWinnings(Mobile m, int amount)
        {
            ...
            if (m.Backpack != null && amount > 0)
            {
                ...
                check.Name = String.Format(XmlPoints.GetText(m, 100300), this.ChallengeName); // "Prize from {0}"
        ...
        public virtual void StartGame()
        {
            ...
            this.GameBroadcast(100002); // "Let the games begin!"
        ...
        public void StartChallengeTimer()
        {
            ...
            this.m_Timer = new ChallengeTimer(this, TimeSpan.FromSeconds(1));
        ...
        {
            if (from == null || this.Organizers == null)
                return false;

            foreach (Mobile m in this.Organizers)
            {
                if (from == m)
                    return true;
            }
        ...
        public override void OnDelete()
        {
            ...
            if (this.GameInProgress)
            {
                this.GameBroadcast(100003, this.ChallengeName);  // "{0} cancelled"
                ...
                if (this.Participants != null)
                {
                    foreach (IChallengeEntry entry in this.Participants)
                    {
                        ...
                        if (from != null && from.BankBox != null && this.EntryFee > 0)
                        {
                            Item gold = new Gold(this.EntryFee);

                            if (!from.BankBox.TryDropItem(from, gold, false))
                            {
                                gold.Delete();
                                from.AddToBackpack(new BankCheck(this.EntryFee));
                                XmlPoints.SendText(from, 100000, this.EntryFee); // "Entry fee of {0} gold has been returned to you."
                            }
                            else
                            {
                                XmlPoints.SendText(from, 100001, this.EntryFee);  // "Entry fee of {0} gold has been returned to your bank account."
                            }
                        }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // version 1
            writer.Write(this.m_IsInChallengeGameRegion);
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            {
                case 1:
                    this.m_IsInChallengeGameRegion = reader.ReadBool();
                    break;
            }
        }
        ...
        public class ChallengeTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                if (this.m_gauntlet != null && !this.m_gauntlet.Deleted && this.m_gauntlet.GameInProgress)
                {
                    this.m_gauntlet.OnTick();
                }
                else
                {
                    this.Stop();
