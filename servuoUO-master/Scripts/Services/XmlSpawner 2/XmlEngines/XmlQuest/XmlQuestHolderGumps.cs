...
namespace Server.Gumps
{
    public class XmlQuestStatusGump : Gump
    {
        ...
        private readonly string m_gumptitle;
        private readonly int m_X;
        private readonly int m_Y;
        private readonly bool m_solid;
        private readonly int m_screen;
            ...
            : base(X, Y)
        {
            ...
            {
                this.AddBackground(54, 33, 369, 400, 5054);
            }
            ...
            this.AddHtmlLocalized(139, 59, 200, 30, 1046026, 0x7fff, false, false); // Quest Log
            this.AddImage(97, 49, 9005); // quest ribbon

            this.AddImageTiled(58, 39, 29, 390, 10460); // left hand border
            this.AddImageTiled(412, 37, 31, 389, 10460); // right hand border
            this.AddImage(430, 9, 10441);
            this.AddImageTiled(40, 38, 17, 391, 9263);
            this.AddImage(6, 25, 10421);
            this.AddImage(34, 12, 10420);
            this.AddImageTiled(94, 25, 342, 15, 10304); // top border
            this.AddImageTiled(40, 414, 415, 16, 10304); // bottom border
            this.AddImage(-10, 314, 10402);
            this.AddImage(56, 150, 10411);

            this.AddImage(136, 84, 96);
            this.AddImage(372, 57, 1417);
            this.AddImage(381, 66, 5576);

            // add the status and journal tabs
            this.AddImageTiled(90, 34, 322, 5, 0x145E); // top border
            int tab1 = 0x138F;
            int tab2 = 0x138E;
            if (screen == 1)
            {
            ...
            this.AddButton(100, 18, tab1, tab2, 900, GumpButtonType.Reply, 0);
            this.AddLabel(115, 17, 0, "Status");
            this.AddButton(189, 18, tab2, tab1, 901, GumpButtonType.Reply, 0);
            this.AddLabel(205, 17, 0, "Journal");
            ...
            if (screen == 1)
            {
                ...
                if (questitem.Journal != null && questitem.Journal.Count > 0)
                {
                    ...
                    this.AddHtml(100, 90, 270, 300, journaltext, true, true);
                }

                // add the add journal entry button
                this.AddButton(300, 49, 0x99C, 0x99D, 952, GumpButtonType.Reply, 0);
            ...
            else
            {
                ...
                { // display the title if it is there
                    this.AddImage(146, 91, 2103); // bullet
                    this.AddHtml(164, 86, 200, 30, XmlSimpleGump.Color(gumptitle, "00FF42"), false, false);
                }

                if (questitem.NoteString != null && questitem.NoteString.Length > 0)
                { // display the note string if it is there
                    this.AddHtml(100, 106, 270, 80, questitem.NoteString, true, true);
                }

                this.DisplayQuestStatus(130, 192, questitem.Objective1, questitem.State1, questitem.Completed1, questitem.Description1);
                this.DisplayQuestStatus(130, 224, questitem.Objective2, questitem.State2, questitem.Completed2, questitem.Description2);
                this.DisplayQuestStatus(130, 256, questitem.Objective3, questitem.State3, questitem.Completed3, questitem.Description3);
                this.DisplayQuestStatus(130, 288, questitem.Objective4, questitem.State4, questitem.Completed4, questitem.Description4);
                this.DisplayQuestStatus(130, 320, questitem.Objective5, questitem.State5, questitem.Completed5, questitem.Description5);

                //if(questitem.HasCollect){
                this.AddButton(100, 350, 0x2A4E, 0x2A3A, 700, GumpButtonType.Reply, 0);
                this.AddLabel(135, 356, 0x384, "Collect");
                //}

                if ((questitem.RewardItem != null && !questitem.RewardItem.Deleted))
                {
                    this.m_questitem.CheckRewardItem();

                    if (questitem.RewardItem.Amount > 1)
                    {
                        this.AddLabel(230, 356, 55, String.Format("Reward: {0} ({1})", questitem.RewardItem.GetType().Name,
                            questitem.RewardItem.Amount));
                        this.AddLabel(230, 373, 55, String.Format("Weight: {0}", questitem.RewardItem.Weight * questitem.RewardItem.Amount));
                    }
                    else if (questitem.RewardItem is Container)
                    {
                        this.AddLabel(230, 356, 55, String.Format("Reward: {0} ({1} items)", questitem.RewardItem.GetType().Name,
                            questitem.RewardItem.TotalItems));
                        this.AddLabel(230, 373, 55, String.Format("Weight: {0}", questitem.RewardItem.TotalWeight + questitem.RewardItem.Weight));
                    }
                    else
                    {
                        this.AddLabel(230, 356, 55, String.Format("Reward: {0}", questitem.RewardItem.GetType().Name));
                        this.AddLabel(230, 373, 55, String.Format("Weight: {0}", questitem.RewardItem.Weight));
                    }
                    this.AddImageTiled(330, 373, 81, 40, 200);
                    this.AddItem(340, 376, questitem.RewardItem.ItemID);
                }
                if (questitem.RewardAttachment != null && !questitem.RewardAttachment.Deleted)
                {
                    this.AddLabel(230, 339, 55, String.Format("Bonus: {0}", questitem.RewardAttachment.GetType().Name));
                }
                ...
                if ((questitem.RewardItem != null && !questitem.RewardItem.Deleted) || (questitem.RewardAttachment != null && !questitem.RewardAttachment.Deleted))
                {
                    ...
                    {
                        this.AddButton(400, 380, 2103, 2103, 800, GumpButtonType.Reply, 0);
                    }
                }
                ...
                {
                    this.AddLabel(100, 392, 33, questitem.Status);
                }
                else if (questitem.IsValid)
                {
                    //AddHtmlLocalized(150, 400, 50, 37, 1046033, 0xf0000 , false , false ); // Expires
                    this.AddHtml(130, 392, 200, 37, XmlSimpleGump.Color(questitem.ExpirationString, "00FF42"), false, false);
                }
                else if (questitem.AlreadyDone)
                {
                    if (!questitem.Repeatable)
                    {
                        this.AddLabel(100, 392, 33, "Already done - cannot be repeated");
                    }
                    else
                    {
                        ...
                        if (a != null && a.Count > 0)
                        {
                            this.AddLabel(100, 392, 33, String.Format("Repeatable in {0}", ((XmlQuestAttachment)a[0]).Expiration));
                        }
                        else
                        {
                            this.AddLabel(150, 392, 33, "Already done - ???");
                        }
                    }
                }
                else
                {
                    ...
                    this.AddLabel(150, 392, 33, "No longer valid");
                }
                if (XmlQuest.QuestPointsEnabled)
                {
                    this.AddHtml(250, 40, 200, 30, XmlSimpleGump.Color(String.Format("Difficulty Level {0}", questitem.Difficulty), "00FF42"), false, false);
                }
                if (questitem.PartyEnabled)
                {
                    this.AddHtml(250, 55, 200, 30, XmlSimpleGump.Color("Party Quest", "00FF42"), false, false);
                    if (questitem.PartyRange >= 0)
                    {
                        this.AddHtml(250, 70, 200, 30, XmlSimpleGump.Color(String.Format("Party Range {0}", questitem.PartyRange), "00FF42"), false, false);
                    }
                    else
                    {
                        this.AddHtml(250, 70, 200, 30, XmlSimpleGump.Color("No Range Limit", "00FF42"), false, false);
                    }
                }
                else
                {
                    this.AddHtml(250, 55, 200, 30, XmlSimpleGump.Color("Solo Quest", "00FF42"), false, false);
                }
            }
        }
        ...
        public void DisplayQuestStatus(int x, int y, string objectivestr, string statestr, bool status, string descriptionstr)
        {
            if (objectivestr != null && objectivestr.Length > 0)
            {
                ...
                this.AddHtml(x, y, 223, 35, XmlSimpleGump.Color(text, "EFEF5A"), false, false);

                if (status)
                {
                    this.AddImage(x - 20, y + 3, 0x939); // bullet
                    this.AddHtmlLocalized(x + 222, y, 225, 37, 1046033, 0xff42, false, false); // Complete
                }
                else
                {
                    this.AddImage(x - 20, y + 3, 0x938); // bullet
                    this.AddHtmlLocalized(x + 222, y, 225, 37, 1046034, 0x7fff, false, false); // Incomplete
                }
            }
        }
        ...
        {
            if (info == null || state == null || state.Mobile == null || state.Mobile.Deleted || this.m_questitem == null || this.m_questitem.Deleted)
                return;
            switch (info.ButtonID)
            {
                case 700:
                    state.Mobile.Target = new XmlQuest.GetCollectTarget(this.m_questitem);
                    state.Mobile.SendGump(new XmlQuestStatusGump(this.m_questitem, this.m_gumptitle, this.m_X, this.m_Y, this.m_solid, this.m_screen));
                    break;
                case 800:
                    if (this.m_questitem.RewardItem != null || this.m_questitem.RewardAttachment != null)
                    {
                        // open a new status gump
                        state.Mobile.SendGump(new XmlQuestStatusGump(this.m_questitem, this.m_gumptitle, this.m_X, this.m_Y, this.m_solid, this.m_screen));
                    }
                    // display the reward item
                    if (this.m_questitem.RewardItem != null)
                    {
                        // show the contents of the xmlquest pack
                        if (this.m_questitem.Pack != null)
                            this.m_questitem.Pack.DisplayTo(state.Mobile);
                    }
                    // identify the reward attachment
                    if (this.m_questitem.RewardAttachment != null)
                    {
                        ...
                        state.Mobile.SendGump(new DisplayAttachmentGump(state.Mobile, this.m_questitem.RewardAttachment.OnIdentify(state.Mobile)));
                    }
                    break;
                case 900:
                    // open a new status gump with status display
                    state.Mobile.SendGump(new XmlQuestStatusGump(this.m_questitem, this.m_gumptitle, this.m_X, this.m_Y, this.m_solid, 0));
                    break;
                case 901:
                    // open a new status gump with journal display
                    state.Mobile.SendGump(new XmlQuestStatusGump(this.m_questitem, this.m_gumptitle, this.m_X, this.m_Y, this.m_solid, 1));
                    break;
                case 952:
                    // open a new status gump with journal display
                    state.Mobile.SendGump(new XmlQuestStatusGump(this.m_questitem, this.m_gumptitle, this.m_X, this.m_Y, this.m_solid, 1));
                    ...
                    if (this.m_questitem.Owner == state.Mobile)
                    {
                        state.Mobile.SendGump(new JournalEntryGump(this.m_questitem, this.m_gumptitle, this.m_X, this.m_Y, this.m_solid));
                    }
                    break;
            }
        }
        ...
        public class JournalEntryGump : Gump
        {
                ...
                : base(X, Y)
            {
                ...
                this.AddImage(0, 0, 0x24AE); // left top scroll
                this.AddImage(114, 0, 0x24AF); // middle top scroll
                this.AddImage(170, 0, 0x24B0); // right top scroll
                this.AddImageTiled(0, 140, 114, 100, 0x24B1); // left middle scroll
                this.AddImageTiled(114, 140, 114, 100, 0x24B2); // middle middle scroll
                this.AddImageTiled(170, 140, 114, 100, 0x24B3); // right middle scroll
                this.AddImage(0, 210, 0x24B4); // left bottom scroll
                this.AddImage(114, 210, 0x24B5); // middle bottom scroll
                this.AddImage(170, 210, 0x24B6); // right bottom scroll
                //AddImageTiled(23, 40, 214, 290, 0x52);
                //AddImageTiled(24, 41, 213, 281, 0xBBC);

                // OK button
                this.AddButton(25, 327, 0xFB7, 0xFB9, 1, GumpButtonType.Reply, 0);
                // Close button
                this.AddButton(230, 327, 0xFB1, 0xFB3, 0, GumpButtonType.Reply, 0);
                // Edit button
                ...
                this.AddTextEntry(35, 5, 200, 21, 0, 1, m_entryid);

                // main text entry area
                this.AddTextEntry(35, 40, 200, 271, 0, 0, str);
            ...
            public override void OnResponse(NetState state, RelayInfo info)
            {
                ...
                if (update_entry)
                {
                    ...
                    this.m_questitem.AddJournalEntry = entryid + ":" + entrytext;
                }
        ...
        private class DisplayAttachmentGump : Gump
        {
                ...
                : base(0, 0)
            {
                ...
                this.AddLabel(20, 2, 55, "Quest Attachment Description");
