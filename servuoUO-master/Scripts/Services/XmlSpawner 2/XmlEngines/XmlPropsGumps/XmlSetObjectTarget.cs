...
namespace Server.Gumps
{
    public class XmlSetObjectTarget : Target
    {
        ...
        public XmlSetObjectTarget(PropertyInfo prop, Mobile mobile, object o, Stack stack, Type type, int page, ArrayList list)
        ...
        {
            if (this.m_Type == typeof(Type))
                from.SendGump(new XmlPropertiesGump(this.m_Mobile, this.m_Object, this.m_Stack, this.m_List, this.m_Page));
            else
                from.SendGump(new XmlSetObjectGump(this.m_Property, this.m_Mobile, this.m_Object, this.m_Stack, this.m_Type, this.m_Page, this.m_List));
        }
    }
}