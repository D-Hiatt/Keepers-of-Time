...
namespace Server.Gumps
{
    public class XmlSetPoint2DGump : Gump
    {
        ...
        public XmlSetPoint2DGump(PropertyInfo prop, Mobile mobile, object o, Stack stack, int page, ArrayList list)
        ...
        private class InternalTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (p != null)
                {
                    try
                    {
                        CommandLogging.LogChangeProperty(this.m_Mobile, this.m_Object, this.m_Property.Name, new Point2D(p).ToString());
                        this.m_Property.SetValue(this.m_Object, new Point2D(p), null);
                    }
                    catch
                    {
                        this.m_Mobile.SendMessage("An exception was caught. The property may not have changed.");
