...
namespace Server.Engines.XmlSpawner2
{
    public class XmlMobFactionsRewards
    {
        ...
        private static readonly ArrayList MobFactionsRewardList = new ArrayList();
        public XmlMobFactionsRewards(string faction, int minfaction, Type reward, string name, int cost, int id, object[] args)
        {
            ...
            this.RequiredFaction = XmlMobFactions.GroupTypes.End_Unused;
            if (faction != null)
                try
                {
                    this.RequiredFaction = (XmlMobFactions.GroupTypes)Enum.Parse(typeof(XmlMobFactions.GroupTypes), faction);
                }
                catch
                {
                }
            this.MinFaction = minfaction;
        }
