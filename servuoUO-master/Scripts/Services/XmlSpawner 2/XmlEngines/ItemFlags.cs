...
namespace Server.Items
{
    public class ItemFlags
    {
        ...
        public static void GetFlag_OnCommand(CommandEventArgs e)
        {
            ...
            if (e.Arguments.Length > 0)
            {
                if (e.Arguments[0].StartsWith("0x"))
                {
                    try
                    {
                        flag = Convert.ToInt32(e.Arguments[0].Substring(2), 16);
                    }
                    catch
                    {
                        error = true;
        ...
        private class GetFlagTarget : Target
        {
            ...
            public GetFlagTarget(CommandEventArgs e, int flag)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Item)
                {
                    bool state = ((Item)targeted).GetSavedFlag(this.m_flag);

                    from.SendMessage("Flag (0x{0:X}) = {1}", this.m_flag, state);
                }
                else
                {
        ...
        private class SetStealableTarget : Target
        {
            ...
            public SetStealableTarget(CommandEventArgs e, bool state)
                : base(30, false, TargetFlags.None)
            {
                this.m_e = e;
                this.m_state = state;
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Item)
                {
                    if (this.set)
                    {
                        SetStealable(((Item)targeted), this.m_state);
                    }
