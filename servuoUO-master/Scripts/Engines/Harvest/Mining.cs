			...
			if ( tool is GargoylesPickaxe && def == m_OreAndStone && 0.1 > Utility.RandomDouble() )
			{
				...
				if ( res == resource && res.Types.Length >= 3 )
				{
					try
					{
						...
						BaseCreature spawned = null;//Activator.CreateInstance( res.Types[2], new object[]{ 25 } ) as BaseCreature;
						try
						{
							int i_Level = CraftResources.GetIndex(CraftResources.GetFromType(res.Types[2])) + 1;
							if (i_Level > 1)
