		...
		protected override void OnTarget( Mobile from, object target )
		{
			...
			else if (target is BaseCreature)
			{
				if (m_Amount >= 10000)
				{
					...
					{
						c.Hue = m_Item.Hue;
						from.Backpack.ConsumeTotal( typeof( Gold ), 10000 ); // Delete the gold
				...
				else if (b_Amount >= 10000)
				{
					...
					if (c.Controlled && c.ControlMaster == from)
					{
						c.Hue = m_Item.Hue;
						from.Backpack.ConsumeTotal( typeof( Gold ), 10000 ); // Delete the gold
						from.SendMessage( "Removed 10k from your bank and hued your pet." );
					}
					else
						from.SendMessage("You can only dye animals whom you control!");
				}
		...
		{
			{
				...
				from.SendMessage( "& You may also hue your pets for 10,000 gp each." );
			}
		}
