...
{
	public class BannerOfNewMaginciaAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfNewMaginciaAddon() : base( 0x4B69 )
		{
			Movable = false;
		}

		public BannerOfNewMaginciaAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfNewMaginciaDeed(); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfNewMaginciaAddonGump ) );
					from.SendGump( new BannerOfNewMaginciaAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfNewMaginciaAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfNewMaginciaAddon m_Addon;

			public BannerOfNewMaginciaAddonGump( Mobile from, BannerOfNewMaginciaAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfNewMaginciaDeed : Item
	{
		public override int LabelNumber{ get{ return 1098177; } } //Banner of NewMagincia

		[Constructable]
		public BannerOfNewMaginciaDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfNewMaginciaDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfNewMaginciaDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfNewMaginciaAddon();
		...
		private class BannerOfNewMaginciaDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfNewMaginciaDeed m_Deed;

			public BannerOfNewMaginciaDeedGump( Mobile from, Point3D loc, BannerOfNewMaginciaDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
