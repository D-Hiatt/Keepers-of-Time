...
{
	public class BannerOfMinocAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfMinocAddon() : base( 0x4B67 )
		{
			Movable = false;
		}

		public BannerOfMinocAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfMinocDeed(); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfMinocAddonGump ) );
					from.SendGump( new BannerOfMinocAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfMinocAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfMinocAddon m_Addon;

			public BannerOfMinocAddonGump( Mobile from, BannerOfMinocAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfMinocDeed : Item
	{
		public override int LabelNumber{ get{ return 1098175; } } //Banner of Minoc

		[Constructable]
		public BannerOfMinocDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfMinocDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfMinocDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfMinocAddon();
		...
		private class BannerOfMinocDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfMinocDeed m_Deed;

			public BannerOfMinocDeedGump( Mobile from, Point3D loc, BannerOfMinocDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
