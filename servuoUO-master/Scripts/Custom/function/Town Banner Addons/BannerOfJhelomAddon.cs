...
{
	public class BannerOfJhelomAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfJhelomAddon() : base( 0x4B65 )
		{
			Movable = false;
		}

		public BannerOfJhelomAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfJhelomDeed(); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfJhelomAddonGump ) );
					from.SendGump( new BannerOfJhelomAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfJhelomAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfJhelomAddon m_Addon;

			public BannerOfJhelomAddonGump( Mobile from, BannerOfJhelomAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfJhelomDeed : Item
	{
		public override int LabelNumber{ get{ return 1098182; } } //Banner of Jhelom

		[Constructable]
		public BannerOfJhelomDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfJhelomDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfJhelomDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfJhelomAddon();
		...
		private class BannerOfJhelomDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfJhelomDeed m_Deed;

			public BannerOfJhelomDeedGump( Mobile from, Point3D loc, BannerOfJhelomDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
