...
{
	public class BannerOfBritainAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfBritainAddon() : base( 0x4B64 )
		{
			Movable = false;
		}

		public BannerOfBritainAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfBritainDeed(); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfBritainAddonGump ) );
					from.SendGump( new BannerOfBritainAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfBritainAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfBritainAddon m_Addon;

			public BannerOfBritainAddonGump( Mobile from, BannerOfBritainAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfBritainDeed : Item
	{
		public override int LabelNumber{ get{ return 1098181; } } // Banner of Britain

		[Constructable]
		public BannerOfBritainDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfBritainDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfBritainDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfBritainAddon();
		...
		private class BannerOfBritainDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfBritainDeed m_Deed;

			public BannerOfBritainDeedGump( Mobile from, Point3D loc, BannerOfBritainDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
