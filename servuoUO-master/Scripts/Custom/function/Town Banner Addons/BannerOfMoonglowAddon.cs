...
{
	public class BannerOfMoonglowAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfMoonglowAddon() : base( 0x4B63 )
		{
			Movable = false;
		}

		public BannerOfMoonglowAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfMoonglowDeed(); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfMoonglowAddonGump ) );
					from.SendGump( new BannerOfMoonglowAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfMoonglowAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfMoonglowAddon m_Addon;

			public BannerOfMoonglowAddonGump( Mobile from, BannerOfMoonglowAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfMoonglowDeed : Item
	{
		public override int LabelNumber{ get{ return 1098180; } } // Banner of Moonglow

		[Constructable]
		public BannerOfMoonglowDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfMoonglowDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfMoonglowDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfMoonglowAddon();
		...
		private class BannerOfMoonglowDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfMoonglowDeed m_Deed;

			public BannerOfMoonglowDeedGump( Mobile from, Point3D loc, BannerOfMoonglowDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
