...
{
	public class BannerOfVesperAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfVesperAddon() : base( 0x4B68 )
		{
			Movable = false;
		}

		public BannerOfVesperAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfVesperDeed(); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfVesperAddonGump ) );
					from.SendGump( new BannerOfVesperAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfVesperAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfVesperAddon m_Addon;

			public BannerOfVesperAddonGump( Mobile from, BannerOfVesperAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfVesperDeed : Item
	{
		public override int LabelNumber{ get{ return 1098176; } } //Banner of Vesper

		[Constructable]
		public BannerOfVesperDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfVesperDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfVesperDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfVesperAddon();
		...
		private class BannerOfVesperDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfVesperDeed m_Deed;

			public BannerOfVesperDeedGump( Mobile from, Point3D loc, BannerOfVesperDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
