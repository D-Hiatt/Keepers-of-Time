...
{
	public class BannerOfTrinsicAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfTrinsicAddon() : base( 0x4B62 )
		{
			Movable = false;
		}

		public BannerOfTrinsicAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfTrinsicDeed( this.Hue ); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfTrinsicAddonGump ) );
					from.SendGump( new BannerOfTrinsicAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfTrinsicAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfTrinsicAddon m_Addon;

			public BannerOfTrinsicAddonGump( Mobile from, BannerOfTrinsicAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfTrinsicDeed : Item
	{
		public override int LabelNumber{ get{ return 1098170; } } // Banner of Trinsic

		[Constructable]
		public BannerOfTrinsicDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfTrinsicDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfTrinsicDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfTrinsicAddon();
		...
		private class BannerOfTrinsicDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfTrinsicDeed m_Deed;

			public BannerOfTrinsicDeedGump( Mobile from, Point3D loc, BannerOfTrinsicDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
