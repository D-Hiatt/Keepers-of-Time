...
{
	public class BannerOfSkaraBraeAddon : Item, IAddon
	{

		[Constructable]
		public BannerOfSkaraBraeAddon() : base( 0x4B6A )
		{
			Movable = false;
		}

		public BannerOfSkaraBraeAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new BannerOfSkaraBraeDeed(); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( BannerOfSkaraBraeAddonGump ) );
					from.SendGump( new BannerOfSkaraBraeAddonGump( from, this ) );
				}
				else
				{
		...
		private class BannerOfSkaraBraeAddonGump : Gump
		{
			private Mobile m_From;
			private BannerOfSkaraBraeAddon m_Addon;

			public BannerOfSkaraBraeAddonGump( Mobile from, BannerOfSkaraBraeAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class BannerOfSkaraBraeDeed : Item
	{
		public override int LabelNumber{ get{ return 1098178; } } //Banner of SkaraBrae

		[Constructable]
		public BannerOfSkaraBraeDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public BannerOfSkaraBraeDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new BannerOfSkaraBraeDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new BannerOfSkaraBraeAddon();
		...
		private class BannerOfSkaraBraeDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private BannerOfSkaraBraeDeed m_Deed;

			public BannerOfSkaraBraeDeedGump( Mobile from, Point3D loc, BannerOfSkaraBraeDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
