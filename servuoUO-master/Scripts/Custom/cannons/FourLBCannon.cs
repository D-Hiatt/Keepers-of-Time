...
{
	public class FourLBCannon : AddonComponent
	{

		public FourLBCannon( int itemID ) : base( itemID )
		{			
			LootType = LootType.Blessed;
		}

		public FourLBCannon( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( Addon is FourCannonAddon )
			{
				if ( ((FourCannonAddon) Addon).IsRewardItem )
					list.Add( 1076223 ); // 7th Year Veteran Reward
					
				list.Add( 1076207, ((FourCannonAddon) Addon).Charges.ToString() ); // Remaining Charges: ~1_val~
			}
		}
	...
	public class FourCannonAddon : BaseAddon
	{	
		public override BaseAddonDeed Deed
		{
			get
			{
				FourCannonDeed deed = new FourCannonDeed();
		...
		public FourCannonAddon( Serial serial ) : base( serial )
		{
		}
		...
		{
			private FourCannonAddon m_Cannon;

			public InternalTarget( FourCannonAddon cannon  ) : base( 12, true, TargetFlags.None )
			{
				m_Cannon = cannon;
			}
		...
		private class InternalGump : Gump
		{
			...
			public InternalGump( FourCannonAddon cannon, PotionKeg keg ) : base( 50, 50 )
			{
				m_Cannon = cannon;
				m_Keg = keg;
	...
	public class FourCannonDeed : BaseAddonDeed, IRewardItem, IRewardOption
	{
		...
		{ 
			get
			{
				FourCannonAddon addon = new FourCannonAddon( m_Direction );
		...
		[Constructable]
		public FourCannonDeed() : base()
		{		
			LootType = LootType.Blessed;
		}

		public FourCannonDeed( Serial serial ) : base( serial )
		{
		}
