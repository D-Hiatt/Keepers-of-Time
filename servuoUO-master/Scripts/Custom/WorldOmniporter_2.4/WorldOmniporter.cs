...
///03.08.2014: Modified gumps to handles additional map locations.
///
...
namespace Server.Items
{
	...
	public class WorldOmniporter : Item
	{
		...
		public enum OptFlags
		{
			...
			TerMur				= 0x000D0020, ///-- When 'true' transport is allowed to the 'TerMur' region
			...
			WO_TimeLimit		= 0x09040000, //##### -- % -- When 'true' the Omniporter will disappear at the end of the TimeExpiration
			WO_UsesLimit		= 0x09080000, //##### -- % -- When 'true' the Omniporter will disappear after MaxUses
		...
		private static void DeleteExpiredOmni()
		{
			...
			foreach ( Item item in World.Items.Values )
			{
				if ( item is WorldOmniporter && !item.Movable ) 
				{
					...
					if ( wo.WO_TimeLimit && ( wo.Birth < ( DateTime.UtcNow - wo.TimeExpiration ) ) ) olist.Add( wo );
				}
			}
		...
		public UseEntry GetUseEntry( Mobile m )
		{
			...
			return ( (UseEntry)GlobalMobileUse["0"] );// TODO: Unreachable code
		}
		...
		[CommandProperty(AccessLevel.Administrator)]
		public TimeSpan TimeExpiration{ get{ return UseGlobal? m_GlobalTimeExpiration: m_TimeExpiration; }
							set{ if (UseGlobal) m_GlobalTimeExpiration = value; else m_TimeExpiration = value; } }
		
		[CommandProperty(AccessLevel.Administrator)]
		public int MaxUses{ get{ return UseGlobal? m_GlobalMaxUses: m_MaxUses; }
	...
	public class WorldOmniporterGump : Gump
	{
		...
		public WorldOmniporterGump( Mobile from, WorldOmniporter WO, int page ) : base( 100, 100 )
		{
			...
			if ( page < 0 || page > 13 ) // If you add a page, you need to raise the last number here....
				page = 0;

			AddPage( 0 );
			if ( m_Staff ) AddBackground( 0, 0, 740, 414, 3500 ); 		// background for staff
			else AddBackground( 0, 0, 740, 414, 3500 ); 				// background for players

			if ( m_Staff ) AddImage(344, 10, 5528); 					// map for staff
			else AddImage(344, 10, 5528); 								// map for players
			AddImage(45, 30, 5609); 									// globe
			if ( m_Staff ) AddLabel(276, 5, 0, "To");					// staff see Omniporter usage
			if ( m_Staff ) AddLabel(316, 5, 0, "From");					// staff see Omniporter usage
			...
			if ( WO.PublicMoongates && (WO.Felucca || (WO.Trammel && m_Reds)) )
			{
				...
				AddPageButton( "<basefont color=#FFFFFF>Public <basefont color=#FFFFFF>Moongates</basefont>", null, p++, 5 );
			}
				...
				if ( WO.Custom )
			{
				...
				AddPageButton( "Custom Locations", null, p++, 11 );
			}
			
			if ( from.AccessLevel > AccessLevel.Player )
			{
				GenerateMapListing( 12 );
				AddPageButton( "Staff Only", null, p++, 12 );
			}
		}
		...
		private void GenerateMapListing( int page )
		{
			...
			for (int i = 0, l = 0; i < oe.Length; i++ )
			{
				...
				else
				{
					...
					if ( m_Staff ) AddLabel( 285, 20+(l*20), 0, entry.ToCount.ToString() );
					if ( m_Staff ) AddLabel( 325, 20+(l*20), 0, entry.FromCount.ToString() );
		...
		public void DoTravel( Mobile from )
		{
			...
			m_Entry.ToCount += go;
			source.FromCount += go;
			// Console.WriteLine( String.Format( "Source: {0}, Map: {1} FromCount now {2}.",
				// source.Name, source.Map.ToString(), source.FromCount.ToString() ) );
			// Console.WriteLine( String.Format( "Destination: {0}, Map: {1} ToCount now {2}.",
				// m_Entry.Name, m_Entry.Map.ToString(), m_Entry.ToCount.ToString() ) );
