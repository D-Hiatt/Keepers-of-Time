...
namespace daat99
{
	public class OWLTROptionsManager
	{
		...
		public void InitOwltrOptions()
		{
			...
			AddOption(OPTIONS_ENUM.DAAT99_MINING, new OWLTROption("Daat99 Mining", "Enabling Daat99 Mining will Make the players mine random ores (according to their skills) everywhere. Disabling it will use the standard OSI vein mining system (they could camp platinum vein 24/7).", true));
			AddOption(OPTIONS_ENUM.DAAT99_LUMBERJACKING, new OWLTROption("Daat99 Lumberjacking", "Enabling Daat99 Lumberjacking will Make the players lumberjack random woods (according to their skills) everywhere. Disabling it will use the standard OSI vein lumberjacking system (they could camp petrified vein 24/7).", true));
			...
			AddOption(OPTIONS_ENUM.MULE_FORGE, new OWLTROption("Mule Forge", "Enabling Mule Forge will enable the mules that spawned with Master Of The Arts to smelt ores just like normal forge, dblclick the ore and target the mule that you own.", true));
			...
			AddOption(OPTIONS_ENUM.MONSTER_GIVE_TOKENS, new OWLTROption("Monster Give Tokens", "Enabling Monster Give Tokens add tokens to players token ledger (assuming it is enabled in their Master Storage pack) when they kill monsters.", true));
            AddOption(OPTIONS_ENUM.BOD_GIVE_TOKENS, new OWLTROption("Bod Give Tokens", "Enabling Bod Give Tokens add tokens to players token ledger (assuming it is enabled in their Master Storage pack) when they return a full bod to the appropriate vendor.", true));
            AddOption(OPTIONS_ENUM.HARVEST_GIVE_TOKENS, new OWLTROption("Harvest Give Tokens", "Enabling Harvest Give Tokens will add tokens to the players token ledger (assuming it is enabled in their Master Storage pack) when they successfully mine ores or lumberjack wood, Only works with random ores/woods. If ores set to vein you can't get tokens for mining and if lumberjacking is set to vein you won't get tokens for lumberjacking. Players will get 50% more tokens when harvesting in Felucca.", true));
            AddOption(OPTIONS_ENUM.CRAFT_GIVE_TOKENS, new OWLTROption("Craft Give Tokens", "Enabling Craft Give Tokens will add tokens to the players token ledger (assuming it is enabled in their Master Storage pack) when they successfully craft items. Tokens amount will be based on the amount of the main resource, the type of the main resource and a bonus if the item is exceptional, When the player repairs an item they'll get 0-4 tokens.", true));
			AddOption(OPTIONS_ENUM.ALCHEMY_RECIPES, new OWLTROption("Alchemy recipes", "Alchemy requiresrequires recipes", true));
			AddOption(OPTIONS_ENUM.BLACKSMITH_RECIPES, new OWLTROption("Blacksmithy recipes", "Blacksmithyrequires recipes", true));
			AddOption(OPTIONS_ENUM.BOWFLETCH_RECIPES, new OWLTROption("BowFletching recipes", "BowFletchingrequires recipes", true));
			AddOption(OPTIONS_ENUM.CARPENTRY_RECIPES, new OWLTROption("Carpentry recipes", "Carpentryrequires recipes", true));
			AddOption(OPTIONS_ENUM.COOKING_RECIPES, new OWLTROption("Cooking recipes", "Cookingrequires recipes", true));
			AddOption(OPTIONS_ENUM.GLASSBLOWING_RECIPES, new OWLTROption("Glassblowing recipes", "Glassblowingrequires recipes", true));
			AddOption(OPTIONS_ENUM.INSCRIPTION_RECIPES, new OWLTROption("Inscription recipes", "Inscriptionrequires recipes", true));
			AddOption(OPTIONS_ENUM.MASONRY_RECIPES, new OWLTROption("Masonry recipes", "Masonryrequires recipes", true));
			AddOption(OPTIONS_ENUM.TAILORING_RECIPES, new OWLTROption("Tailoring recipes", "Tailoringrequires recipes", true));
			AddOption(OPTIONS_ENUM.TINKERING_RECIPES, new OWLTROption("Tinkering recipes", "Tinkeringrequires recipes", true));
			...
			AddOption(OPTIONS_ENUM.TOKENS_LOTTERY, new OWLTROption("Tokens Lottery", "When enabled it'll allow players to buy lottery tickets from Lady Luck. <br>It won't effect tickets that were sold already and Lady Luck will accept that last drawing tickets as usual.", true));
		...
		{
			int version = reader.ReadInt();// TODO: Unused var
